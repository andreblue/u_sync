...
local MODULE_NOT_EXIST = "[usync-mysql] The %s module does not exist!\n"
...
local function BuildSelectQuery(queryObj)
	...
	if (isstring(queryObj.tableName)) then
		...
		ErrorNoHalt("[usync-mysql] No table name specified!\n")
		return
	end
...
local function BuildAlterQuery(queryObj)
	...
	elseif (isstring(queryObj.drop)) then
		if (mysql.module == "sqlite") then
			ErrorNoHalt("[usync-mysql] Cannot drop columns in sqlite!\n")
			return
		end
...
function mysql:RawQuery(query, callback, flags, ...)
	if (self.module == "mysqloo") then
		...
		queryObj.onSuccess = function(queryObj, result)
			if (callback) then
				...
				if (!bStatus) then
					error(string.format("[usync-mysql] MySQL Callback Error!\n%s\n", value))
				end
			end
		end

		queryObj.onError = function(queryObj, errorText)
			ErrorNoHalt(string.format("[usync-mysql] MySQL Query Error!\nQuery: %s\n%s\n", query, errorText))
		end
	...
	elseif (self.module == "sqlite") then
		...
		if (result == false) then
			error(string.format("[usync-mysql] SQL Query Error!\nQuery: %s\n%s\n", query, sql.LastError()))
		else
			if (callback) then
				local bStatus, value = pcall(callback, result, true, tonumber(sql.QueryValue("SELECT last_insert_rowid()")))

				if (!bStatus) then
					error(string.format("[usync-mysql] SQL Callback Error!\n%s\n", value))
				end
			end
		end
	else
		ErrorNoHalt(string.format("[usync-mysql] Unsupported module \"%s\"!\n", self.module))
	end
end
...
function mysql:OnConnected()
	MsgC(Color(25, 235, 25), "[usync-mysql] Connected to the database!\n")
...
function mysql:OnConnectionFailed(errorText)
	ErrorNoHalt(string.format("[usync-mysql] Unable to connect to the database!\n%s\n", errorText))
